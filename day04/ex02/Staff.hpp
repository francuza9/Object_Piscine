...
class Headmaster : public Staff
{
	...
	public:
		...
		~Headmaster() { for (auto it : _formToValidate) if (it) delete it; }
		void receiveForm(Form* p_form) { _formToValidate.push_back(p_form); p_form->sign(); }
		void executeForm(Form* p_form) { p_form->execute(); }
};
